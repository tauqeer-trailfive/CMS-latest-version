   ...
   useEffect(() => {
      if (!singleContestLoading) {
         if (!!!contestDataUpdated && getContestData) {
            localStorage.removeItem('CONTEST_MEDIA_EDIT_DATA')
            const formatedContestMedia =
               getContestData?.contest?.contestMedia?.map((media) => {
                  const {
                     id,
                     stage,
                     image,
                     ...
                     return (
                        <TabPanel value={value} index={index}>
                           ...
                           <Box flex={1} mr={{ xs: 0, sm: '0.5em' }}>
                              <TextInput
                                 ...
                                 helperText={
                                    field?.data?.stage === 'DURINGCONTEST' ? (
                                       <p style={{ color: 'red' }}>
                                          (Required field): This is Default
                                          Description for all stages.
                                       </p>
                                    ) : (
                                       ''
                                    )
                           ...
                           <Box flex={1} mr={{ xs: 0, sm: '0.5em' }} my={2}>
                              ...
                              >
                                 {Object.keys(config_data_contestMedia).map(
                                    (key) => (
                                       <input
                                          key={key}
                                          name={key}
                              ...
                              {contestMediaFiles1[index].data?.image && (
                                 <>
                                    <TextInput
                                       ...
                                       hidden={true}
                                       inputProps={{
                                          value: contestMediaFiles1[index].data
                                             ?.image,
                                       }}
                                       value={
                                          contestMediaFiles1[index].data?.image
