...
const ProjectsChart = (props: { orders?: Order[] }) => {
   const GET_PROJECT_GRAPH_DATA = gql`
      query GetProjectsGraphData($initialDate: DateTime!) {
         getProjectsGraphData(initialDate: $initialDate) {
            date
            count
         }
      }
   ...
   return (
      <Card elevation={5}>
         ...
         <Box display={{ xs: 'block', sm: 'flex', width: '100%' }} gap={1}>
            <Chip
               label="New Projects Per day - Past 30 days."
               variant="outlined"
               color="primary"
               sx={{ ml: 5, fontSize: 18, fontWeight: '400' }}
               icon={
                  <Badge color="warning">
                     <SsidChartTwoToneIcon color="primary" />
                  </Badge>
               }
            />
         </Box>
               ...
               {loading ? (
                  ...
                  <ResponsiveContainer>
                     <AreaChart
                        width={730}
                        height={250}
                        data={data?.getProjectsGraphData}
                        margin={{
                           top: 10,
                           right: 30,
                           left: 0,
